...
TEST_FILES = ['NO2-test-L1.xlsx', 'NO2-test-L2.xlsx', 'NO2-test-L3.xlsx']

def process_data(file_path):
    df = pd.read_excel(file_path)
    df.columns = ['C', 'R', 'G', 'B']

    # Remove zero denominators
...
for cp in candidate_points:
    ...
    threshold_bg = df_train[df_train['C'] == cp]['B_G'].mean()

    #Prepare training data with dummy variable D
    df_t = df_train.copy()
    df_t['D'] = (df_t['B_G'] > threshold_bg).astype(int)

    df_t['D_x_BG'] = df_t['D'] * df_t['B_G']
    df_t['D_x_GR'] = df_t['D'] * df_t['G_R']
    df_t['D_x_BR'] = df_t['D'] * df_t['B_R']

    X_train = df_t[['B_G', 'D', 'D_x_BG', 'D_x_GR', 'D_x_BR']]
    y_train = df_t['C']
    ...
    for df_test in test_dfs:
        ...
        df_temp['D'] = (df_temp['B_G'] > threshold_bg).astype(int)
        df_temp['D_x_BG'] = df_temp['D'] * df_temp['B_G']
        df_temp['D_x_GR'] = df_temp['D'] * df_temp['G_R']
        df_temp['D_x_BR'] = df_temp['D'] * df_temp['B_R']

        X_test = df_temp[['B_G', 'D', 'D_x_BG', 'D_x_GR', 'D_x_BR']]
...
print(f"{'break-point (ppm)':<10} | {'Training RMSE':<12} | {'Testing MAE':<12} | {'Max Coeff'}")
print("-" * 65)
for r in results:
    mark = " <-- Optimal break-point" if r['cp'] == best_res['cp'] else ""
    print(f"{r['cp']:<10.2f} | {r['train_rmse']:<12.5f} | {r['test_mae']:<12.5f} | {r['stability']:<10.2f}{mark}")
...
plt.axvline(x=best_res['cp'], color='green', linestyle='--',
            label=f'Optimal break-point: {best_res["cp"]} ppm')

plt.xlabel('Concentration break-point (ppm)')
plt.ylabel('Error (ppm)')
plt.title("Generalization-optimal breakpoint determination for NO$_2^-$")
plt.legend()
...
#-----------------------------------------------------------------------------
OUTPUT_FILE = 'Generalization_optimal_breakpoint_NO2.xlsx'
